...
//         return res;
        ...
        ListNode *curr = temp;           // node for traversing the as per the smaller value LL representing temp LL
        while(l1!=NULL && l2!=NULL){ // we travrse until any of the LL points to null
            if(l1->val <= l2->val ){ // comparing the value if the nodes that are pointing
                curr->next = l1; //insert the smaller ll to next of current  
                l1 = l1->next;// traverse forward
            }
            else{
                curr->next = l2;
                l2 = l2->next;
            }
            curr = curr->next; // move the current to next
        }
